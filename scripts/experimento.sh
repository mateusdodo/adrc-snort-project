...
# Matriz Experimental: 5 Níveis de Regras x 2 Cargas x 2 Portas x 30 Réplicas
# ==============================================================================
...
# O laço de REGRAS fica no topo para você só alterar o snort.conf 5 vezes no total
for REGRA in "${REGRAS[@]}"; do
    ...
    for CARGA in "${CARGAS[@]}"; do
        for PORTA in "${PORTAS[@]}"; do
            ...
            # Lógica de validação automatizada baseada no nível de regras
            if [ "$REGRA" == "Baseline" ]; then
                # No Baseline (sem regras), a latência deve ser baixa, mas controlada pelo DAQ
                IS_ERRADO=$(awk -v rtt="$MINI_RTT" 'BEGIN {print (rtt > 4.0) ? 1 : 0}')
                if [ "$IS_ERRADO" -eq 1 ]; then
                    echo "⚠️  [AVISO] Latência acima do esperado para Baseline. Verifique a CPU da VM."
                fi
            else
                # Em cenários com regras ativas, RTT menor que 2.5ms indica BYPASS/CRASH inevitável
                IS_BYPASS=$(awk -v rtt="$MINI_RTT" 'BEGIN {print (rtt < 2.5) ? 1 : 0}')
